...
LtePhy::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::LtePhy")
    ...
    .AddAttribute ("TxTimeInterval",
                   "Transmission time interval",
                   DoubleValue (0.001),
                   //MakeDoubleAccessor (&LtePhy::m_tti,
                   MakeDoubleAccessor (&LtePhy::SetTti,
                                       &LtePhy::GetTti),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("FlexibleTxTimeInterval",                                    //sachin start
                   "Transmission time interval",
                   DoubleValue (0.000499999),
                   //MakeDoubleAccessor (&LtePhy::m_tti,
                   MakeDoubleAccessor (&LtePhy::SetStti,
                                       &LtePhy::GetStti),
                   MakeDoubleChecker<double> ())                               //sachin end
  ;
  return tid;
}
...
void
LtePhy::SetStti (double stti)                   //sachin start
{
  NS_LOG_FUNCTION (this << stti);
  m_stti = stti;
}                                               //sachin end
...
double
LtePhy::GetStti (void) const                    //sachin start
{
  NS_LOG_FUNCTION (this << m_stti);
  return m_stti;
}                                               //sachin end
...
void
LtePhy::SetSfDuration (double sfDuration)
{
  NS_LOG_FUNCTION (this << sfDuration);
  m_sfDuration = sfDuration;
}


double
LtePhy::GetSfDuration (void) const
{
  NS_LOG_FUNCTION (this << m_sfDuration);
  return m_sfDuration;
}

void
LtePhy::SetSlot (double slot)
{
...
LtePhy::GetSrsPeriodicity (uint16_t srcCi) const
{
  ...
  uint16_t SrsPeriodicity[10] = {0, 2, 5, 10, 20, 40, 80, 160, 320, 640};
  uint16_t SrsCiLow[10] = {0, 0, 2, 7, 17, 37, 77, 157, 317, 637};
  uint16_t SrsCiHigh[10] = {0, 1, 6, 16, 36, 76, 156, 316, 636, 1023};
  uint8_t i;
  for (i = 9; i > 0; i --)
    {
