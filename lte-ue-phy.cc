...
//static const Time UL_DATA_DURATION = NanoSeconds (5e5 - 71429 - 1);
//static const Time UL_DATA_DURATION = NanoSeconds (71429 - 1);
...
//static const Time UL_SRS_DELAY_FROM_SUBFRAME_START = NanoSeconds (5e5 - 71429);
...
{
  //std::cout << "UE with RNTI " << raRnti << ", using preambleID " << prachId << std::endl;
...
LteUePhy::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::LteUePhy")
    ...
    .AddAttribute ("EnableUplinkPowerControl",
                   ...
                   MakeBooleanChecker ())
/*    .AddAttribute ("UeMacToChDelay",
                   "Processing delay at UE between MAC and PHY Tx.",
                   UintegerValue (4),
                   MakeUintegerAccessor (&LteUePhy::SetMacChDelay,
                                         &LteUePhy::GetMacChDelay),
                   MakeUintegerChecker<uint8_t> ())*/
   .AddAttribute( "nUe",                                                                     //sachin start
                  "number of UEs for 14 symbol trensmission (default 1)",
                   UintegerValue(1),
                   MakeUintegerAccessor (&LteUePhy::m_nUe),
                   MakeUintegerChecker<uint16_t> ())
    .AddAttribute("kUe",
                  "number of UEs for 7 symbol trensmission (default 1)",
                   UintegerValue(1),
                   MakeUintegerAccessor (&LteUePhy::m_kUe),
                   MakeUintegerChecker<uint16_t> ())                                        //sachin end
  ; 
  return tid;
}
...
/*
void
LteUePhy::SetMacChDelayUe (uint8_t delay)
{
  NS_LOG_FUNCTION (this);
  m_macChTtiDelayUe = delay;
}

uint8_t
LteUePhy::GetMacChDelayUe (void) const
{
  return (m_macChTtiDelayUe);
}
...
LteUePhy::SubframeIndication (uint32_t frameNo, uint32_t subframeNo)
{
  ...
  if (m_ulConfigured)
    {
      ...
      if (pb)
        {
          ...
          //m_uplinkSpectrumPhy->StartTxDataFrame (pb, ctrlMsg, Seconds(GetTti()));
      //  if (0.001 == GetTti())
      //    {
            m_uplinkSpectrumPhy->StartTxDataFrame (pb, ctrlMsg, UL_DATA_DURATION); //only this
      //    }
      //    else
      //    {
      //      m_uplinkSpectrumPhy->StartTxDataFrame (pb, ctrlMsg, Seconds(GetTti()));
      //    }
        ...
        //      if (0.001 == GetTti())
        //      {
                m_uplinkSpectrumPhy->StartTxDataFrame (pb, ctrlMsg, UL_DATA_DURATION); //need only this
        //      }
        //      else
        //      {
        //        m_uplinkSpectrumPhy->StartTxDataFrame (pb, ctrlMsg, Seconds(GetTti()));
        //      }
              //std::cout<< GetTti() << std::endl;
     ...
     if(m_subframeNo %2 == 1)                                                                                   //sachin start
     { 
      if (pb)
        {
          if (m_enableUplinkPowerControl)
          ...
          //m_uplinkSpectrumPhy->StartTxDataFrame (pb, ctrlMsg, UL_DATA_DURATION);
          //m_uplinkSpectrumPhy->StartTxDataFrame (pb, ctrlMsg, Seconds(GetTti()));
       //    if (0.000499999 == GetStti())
       //   {
       //     m_uplinkSpectrumPhy->StartTxDataFrame (pb, ctrlMsg, UL_DATA_DURATION);
       //   }
       //   else
       //   {
            m_uplinkSpectrumPhy->StartTxDataFrame (pb, ctrlMsg, Seconds(GetStti()));                            //only need this
       //   }
        }
      else
        {
         ...
         //     else
         //     {
                m_uplinkSpectrumPhy->StartTxDataFrame (pb, ctrlMsg, Seconds(GetStti()));                         //this one
         //     }
              //std::cout<< GetTti() << std::endl;
  ...
  Simulator::Schedule (Seconds (GetSfDuration ()), &LteUePhy::SubframeIndication, this, frameNo, subframeNo);
}
